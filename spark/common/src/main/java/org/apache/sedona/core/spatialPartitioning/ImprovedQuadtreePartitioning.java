...
public class ImprovedQuadtreePartitioning<T extends Geometry> implements Serializable {

  /** The Improved Quad-Tree. */
  private final ImprovedQuadTree<Integer> partitionTree;

  public ImprovedQuadtreePartitioning(SpatialRDD<T> spatialRDD, int numPartitions)
      throws Exception {
    // ----------start partitioning----------
    spatialRDD.analyze();
    ...
    while (rate >= 1 && maxIter < 3) {
      double tempGridWidth =
          Math.ceil((boundary.getMaxX() - boundary.getMinX()) / gridScale * 10000) / 10000.0;
      double tempGridHeight =
          Math.ceil((boundary.getMaxY() - boundary.getMinY()) / gridScale * 10000) / 10000.0;
      gridXY_countRDD =
          spatialRDD
              .rawSpatialRDD
              .mapToPair(
                  geometry -> {
                    ...
                    assert point != null;
                    int gridX =
                        (point.getX() == boundary.getMinX())
                            ? 0
                            : (int)
                                (Math.ceil(((point.getX() - boundary.getMinX()) / tempGridWidth))
                                    - 1);
                    int gridY =
                        (point.getY() == boundary.getMinY())
                            ? 0
                            : (int)
                                (Math.ceil(((point.getY() - boundary.getMinY()) / tempGridHeight))
                                    - 1);
                ...
                geometry -> {
                  ...
                  int gridX =
                      (point.getX() == boundary.getMinX())
                          ? 0
                          : (int)
                              (Math.ceil(((point.getX() - boundary.getMinX()) / gridWidth)) - 1);
                  int gridY =
                      (point.getY() == boundary.getMinY())
                          ? 0
                          : (int)
                              (Math.ceil(((point.getY() - boundary.getMinY()) / gridHeight)) - 1);
    ...
    for (int i = 1; i <= gridScale; i++) {
      for (int j = 1; j <= gridScale; j++) {
        prefixSum[i][j] =
            prefixSum[i - 1][j] + prefixSum[i][j - 1] - prefixSum[i - 1][j - 1] + gridCounts[i][j];
      }
    }
    gridCounts = null;
    ...
    //            System.out.println("maxItems: " + maxItemsPerZone + " minItems: " +
    // minItemsPerZone);
    //            partitionTree = new ImprovedQuadTree(new QuadRectangle(boundary, 0, 0, gridScale -
    // 1, gridScale - 1), (int) totalCount, 0, maxItemsPerZone, minItemsPerZone, numPartitions,
    // gridWidth, gridHeight,prefixSum);
    //        } else {
    ...
    partitionTree =
        new ImprovedQuadTree(
            new QuadRectangle(boundary, 0, 0, gridScale - 1, gridScale - 1),
            (int) totalCount,
            0,
            maxItemsPerZone,
            minItemsPerZone,
  ...
  static class ValueComparator
      implements Serializable, Comparator<Tuple2<Tuple2<Integer, Integer>, Integer>> {
    @Override
    public int compare(
        Tuple2<Tuple2<Integer, Integer>, Integer> t1,
        Tuple2<Tuple2<Integer, Integer>, Integer> t2) {
      return t1._2 - t2._2;
    }
  }
}

...
public class ImprovedQuadTree<T> extends PartitioningUtils implements Serializable {
  ...
  public ImprovedQuadTree(
      QuadRectangle definition, int level, int maxItemsPerZone, int minItemsPerZone, int maxLevel) {
  ...
  private ImprovedQuadTree<T> newUnevenQuadTree(QuadRectangle zone, int nodeNum, int level) {
    ...
    return new ImprovedQuadTree<T>(
        zone,
        nodeNum,
        level,
        this.maxItemsPerZone,
        this.minItemsPerZone,
        this.maxLevel,
        this.gridWidth,
        this.gridHeight,
        this.prefixSum);
  ...
  private void unevenSplit() {
    ...
    if (zone.xStep * gridWidth >= zone.yStep * gridHeight) {
      ...
      if (regions[0] != null && regions[0].getNodeNum() > maxItemsPerZone) {
        if (regions[0].getZone().xStep * gridWidth >= 2 * regions[0].getZone().yStep * gridHeight)
          splitX(regions[0].getZone(), newLevel, regions[0].getNodeNum(), false, 0);
      ...
      if (regions[2] != null && regions[2].getNodeNum() > maxItemsPerZone) {
        if (regions[2].getZone().xStep * gridWidth >= 2 * regions[2].getZone().yStep * gridHeight)
          splitX(regions[2].getZone(), newLevel, regions[2].getNodeNum(), false, 2);
      ...
      if (regions[0] != null && regions[0].getNodeNum() > maxItemsPerZone) {
        if (regions[0].getZone().yStep * gridHeight >= 2 * regions[0].getZone().xStep * gridWidth)
          splitY(regions[0].getZone(), newLevel, regions[0].getNodeNum(), false, 0);
        else splitX(regions[0].getZone(), newLevel, regions[0].getNodeNum(), false, 0);
      }
      if (regions[2] != null && regions[2].getNodeNum() > maxItemsPerZone) {
        if (regions[2].getZone().yStep * gridHeight >= 2 * regions[2].getZone().xStep * gridWidth)
          splitY(regions[2].getZone(), newLevel, regions[2].getNodeNum(), false, 2);
  ...
  private void splitX(QuadRectangle zone, int level, int nodeSum, boolean isFirst, int index) {
    ...
    for (int i = 0; i <= zone.xStep / 2; i++) {
      ...
      rightNodeSum =
          getzonePrefixSum(right, zone.gridY, zone.gridX + zone.xStep, zone.gridY + zone.yStep);
    ...
    //        System.out.println("gridX: "+zone.gridX +" xStep: " +zone.xStep+" minxAxis:
    // "+minxAxis+" minxSum: "+minxSum+" minLeftNodeSum: "+minLeftNodeSum+" minRightNodeSum:
    // "+minRightNodeSum);
  ...
  private void splitY(QuadRectangle zone, int level, int nodeSum, boolean isFirst, int index) {
    ...
    for (int i = 0; i <= zone.yStep / 2; i++) {
      ...
      topNodeSum =
          getzonePrefixSum(zone.gridX, top, zone.gridX + zone.xStep, zone.gridY + zone.yStep);
    ...
    //        System.out.println("gridY: "+zone.gridY +" yStep: " +zone.yStep+" minyAxis:
    // "+minyAxis+" minySum: "+minySum+" minTopNodeSum: "+minTopNodeSum+" minBottomNodeSum:
    // "+minBottomNodeSum);
  ...
  public Iterator<Tuple2<Integer, Geometry>> placeObject(Geometry geometry) {
    ...
    final List<Pair<QuadRectangle, Integer>> matchedPartitions =
        findZonesWithNum(new QuadRectangle(envelope));
    ...
    for (Pair<QuadRectangle, Integer> rectangle : matchedPartitions) {
      if (point != null) {
        ...
        if (value > maxValue) {
          ...
          // ", " + point.getUserData() + ", " + matchedPartitions);
      ...
      if (geometry instanceof Circle) {
        ...
        if (new HalfOpenRectangle(rectangle.getLeft().getEnvelope())
            .contains(circle.getCenterGeometry().getCentroid())) {
          continue;
        }
      }
    }
  ...
  public Iterator<Tuple2<Integer, Geometry>> placeObjectWithFilter(Geometry geometry) {
    ...
    for (QuadRectangle rectangle : matchedPartitions) {
      ...
      //                Circle circle = (Circle) geometry;
      //                if (new
      // HalfOpenRectangle(rectangle.getEnvelope()).contains(circle.getCenterGeometry().getCentroid())) {
      //                    continue;
      //                }
      //            }
